...
import loadAddressesReducer from "./reducer";
...
describe("FETCH_ADDRESSES_SUCCESS sets store.isLoadingAddresses to false, sets store.myAddresses", () => {
  ...
  const testState = loadAddressesReducer(
    initialState,
    fetchAddressesSuccess(testAdresses)
  );
...
describe("FETCH_ADDRESSES_FAILURE", () => {
  ...
  const testState = loadAddressesReducer(
    initialState,
    fetchAddressesFailure("test error")
  );
  ...
  it("store.errorText -> Ошибка загрузки", () => {
    expect(testState.errorText).toEqual(
